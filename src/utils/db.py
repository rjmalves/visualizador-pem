...
def _create_screen(screen_name: str, screen_type: ScreenType, studies: List[Study]):
    uow = SQLUnitOfWork()
    with uow:
...
def _update_screen(screen_name: str, screen_type: ScreenType, studies: List[Study]):
    _delete_screen(screen_name)
    if len(studies) > 0:
        _create_screen(screen_name, screen_type, studies)


def create_or_update_screen(screen_name: str, screen_type_str: str, current_studies):
...
def load_screen(screen_name: str, screen_type_str: str) -> Optional[pd.DataFrame]:
    ...
    with uow:
        ...
        if len(selected_screens) == 1:
            ...
            if len(studies_ids) > 0:
                studies = [s for s in uow.studies.list() if s.study_id in studies_ids]
                return pd.DataFrame(
                    data={
